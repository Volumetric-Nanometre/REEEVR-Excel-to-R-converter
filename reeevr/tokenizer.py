...
This tokenizer is taken from openpyxl version 3.1.
The tokenizer is edited to correctly handle dynamic ranges such as A5:OFFSET() following work provided in PR 43.
This works by setting : as an infix operator.

This tokenizer file to be used until openpyxl tokenizer is updated to the equivalent level.
...
class Tokenizer(object):
    ...
    def merge_constant_range(self):
        """Since we treat : as infix operator for the case of OFFSET(C1, 1,1):E10 and alike
        for backward compatibility(treating D2:E10 as a single range) the constant range needs
        to be merged in to a single object.
        """
        if len(self.items) >= 3:
            left, op, right = self.items[-3:]
            # Ex: A1:3, A1:A3, 2:2 A:A
            is_constant_range = (left.subtype == Token.RANGE or left.subtype == Token.NUMBER) and \
                                op.type == Token.OP_IN and op.value == ':' and \
                                (right.subtype == Token.RANGE or right.subtype == Token.NUMBER)
            if is_constant_range:
                del self.items[-3:]
                const_range = left.value + op.value + right.value
                self.items.append(Token.make_operand(const_range))
